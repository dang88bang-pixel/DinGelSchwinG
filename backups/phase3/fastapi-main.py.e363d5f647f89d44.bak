...
# ---------------------------------------------------------------------------
# Graph overview (REST) — drives the Android node canvas with live data
# ---------------------------------------------------------------------------
# REAL-IMPLEMENTATION 2026-09-11 (Phase 2.6): same 5 nodes the app used to
# hardcode, kept as an explicit fallback when Neo4j is unreachable.
_DEMO_GRAPH: dict[str, Any] = {
    "nodes": [
        {"id": "switch-a", "label": "Switch A", "type": "switch", "status": "UNKNOWN", "x": 0.5, "y": 0.30},
        {"id": "switch-b", "label": "Switch B", "type": "switch", "status": "UNKNOWN", "x": 0.25, "y": 0.60},
        {"id": "switch-c", "label": "Switch C", "type": "switch", "status": "UNKNOWN", "x": 0.75, "y": 0.60},
        {"id": "vesc-1", "label": "VESC 1", "type": "controller", "status": "UNKNOWN", "x": 0.15, "y": 0.85},
        {"id": "ninebot-1", "label": "Ninebot", "type": "controller", "status": "UNKNOWN", "x": 0.55, "y": 0.90},
    ],
    "edges": [
        {"from": "switch-a", "to": "switch-b", "type": "UPLINK"},
        {"from": "switch-a", "to": "switch-c", "type": "UPLINK"},
        {"from": "switch-b", "to": "vesc-1", "type": "UART"},
        {"from": "switch-c", "to": "ninebot-1", "type": "BLE"},
    ],
}


@app.get("/graph")
async def graph(limit: int = 50) -> dict[str, Any]:
    """Return the node/edge overview for the Android canvas.

    Live data from Neo4j when reachable, otherwise the explicit
    ``demo-fallback`` graph (never an error, never a guess without a label).
    """
    try:
        data = await app.state.neo4j.get_graph_overview(limit)
        if data.get("nodes"):
            return {"source": "neo4j", **data}
        logger.info("Neo4j graph is empty — serving demo fallback")
    except Exception as exc:  # noqa: BLE001 - graceful degradation by design
        logger.warning("Neo4j graph overview failed, using demo fallback: %s", exc)
    return {"source": "demo-fallback", **_DEMO_GRAPH}
