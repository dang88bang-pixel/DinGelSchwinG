      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-white/5 bg-[#060f2a]/60 p-4">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 text-[11px] font-extrabold px-3 py-2 rounded-lg border transition mb-3 ${
              store.snifferActive
                ? 'bg-rose-600 text-white border-rose-500/50'
                : 'bg-slate-800 text-slate-200 border-white/10 hover:bg-slate-700'
            }`}
          >
            ...
            {store.snifferActive ? 'Sniffer stoppen' : 'Sniffer starten'}
          </button>
          <div className="max-h-40 overflow-y-auto space-y-0.5 font-mono text-[9px]">
            {store.snifferPackets.slice(-15).reverse().map((p) => (
            ...
            {store.snifferPackets.length === 0 && (
              <div className="text-slate-600 text-center py-4">Sniffer inaktiv – keine Pakete.</div>
            )}
          </div>
        </div>
